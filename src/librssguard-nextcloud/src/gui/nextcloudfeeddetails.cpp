...
NextcloudFeedDetails::NextcloudFeedDetails(QWidget* parent) : QWidget(parent) {
  ui.setupUi(this);
  ...
  connect(ui.m_txtUrl->lineEdit(), &BaseLineEdit::textChanged, this, &NextcloudFeedDetails::onUrlChanged);
  onUrlChanged(QString());
}

void NextcloudFeedDetails::onUrlChanged(const QString& new_url) {
...
void NextcloudFeedDetails::loadCategories(const QList<Category*>& categories,
                                          RootItem* root_item,
