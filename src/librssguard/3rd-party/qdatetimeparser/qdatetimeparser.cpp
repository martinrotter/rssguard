...
int QDateTimeParser::getDigit(const QDateTime& t, int index) const {
  if (index < 0 || index >= sectionNodes.size()) {
    qWarning("QDateTimeParser::getDigit() Internal error (%ls %d)", qUtf16Printable(t.toString()), index);
    return -1;
  }
  ...
  qWarning("QDateTimeParser::getDigit() Internal error 2 (%ls %d)", qUtf16Printable(t.toString()), index);
  return -1;
}
...
bool QDateTimeParser::setDigit(QDateTime& v, int index, int newVal) const {
  if (index < 0 || index >= sectionNodes.size()) {
    qWarning("QDateTimeParser::setDigit() Internal error (%ls %d %d)", qUtf16Printable(v.toString()), index, newVal);
    return false;
  }
  ...
  switch (node.type) {
    ...
    case AmPmSection:
      ...
      break;
    default:
      qWarning("QDateTimeParser::setDigit() Internal error (%ls)", qUtf16Printable(node.name()));
      break;
  }
  ...
  if (fixDay != NoFix) {
    ...
    Q_ASSERT(fixDay != WeekDay || calendar.dayOfWeek(calendar.dateFromParts(date)) == weekDay);
  }
...
int QDateTimeParser::absoluteMax(int s, const QDateTime& cur) const {
  ...
  qWarning("QDateTimeParser::absoluteMax() Internal error (%ls)", qUtf16Printable(sn.name()));
  return -1;
}
...
int QDateTimeParser::absoluteMin(int s) const {
  ...
  qWarning("QDateTimeParser::absoluteMin() Internal error (%ls, %0x)", qUtf16Printable(sn.name()), sn.type);
  return -1;
}
...
static inline void appendSeparator(QStringList* list, QStringView string, int from, int size, int lastQuote) {
      ...
      switch (sect) {
        ...
        case 'a':
          if (parserType != QMetaType::QDate) {
            ...
            if (i + 1 < newFormat.size() && newFormat.sliced(i + 1).startsWith(u'p', Qt::CaseInsensitive)) {
              ++i;
        ...
        case 'y':
          if (parserType != QMetaType::QTime) {
            ...
            if (repeat >= 2) {
              const SectionNode sn = {repeat == 4 ? YearSection : YearSection2Digits, i - add, repeat == 4 ? 4 : 2, 0};
              newSectionNodes.append(sn);
              appendSeparator(&newSeparators, newFormat, index, i - index, lastQuote);
              i += sn.count - 1;
              index = i + 1;
        ...
        case 'd':
          if (parserType != QMetaType::QTime) {
            ...
            const Section sectionType =
              (repeat == 4 ? DayOfWeekSectionLong : (repeat == 3 ? DayOfWeekSectionShort : DaySection));
...
int QDateTimeParser::sectionMaxSize(Section s, int count) const {
  ...
  switch (s) {
    ...
    case AmPmSection:
      ...
      return qMax(getAmPmText(AmText, Case(count)).size(), getAmPmText(PmText, Case(count)).size());
    ...
    case MonthSection:
...

      {
        ...
        for (int i = 1; i <= mcount; ++i) {
          const QString str =
            (s == MonthSection ? calendar.monthName(l, i, QCalendar::Unspecified, format) : l.dayName(i, format));
    ...
    case DaySectionMask:
      qWarning("QDateTimeParser::sectionMaxSize: Invalid section %s", SectionNode::name(s).toLatin1().constData());
...
QDateTimeParser::ParsedSection QDateTimeParser::parseSection(const QDateTime& currentValue,
                                                             int sectionIndex,
                                                             int offset) const {
  ParsedSection result; // initially Invalid
  const SectionNode& sn = sectionNode(sectionIndex);
  Q_ASSERT_X(!(sn.type & Internal), "QDateTimeParser::parseSection", "Internal error");

  const int sectionmaxsize = sectionMaxSize(sectionIndex);
  const bool negate = (sn.type == YearSection && m_text.size() > offset && m_text.at(offset) == u'-');
  ...
  const QDateTime defaultValue =
    currentValue.isValid() ? currentValue : QDateTime::fromMSecsSinceEpoch(currentValue.toMSecsSinceEpoch());

  QStringView sectionTextRef = QStringView{m_text}.mid(offset + negativeYearOffset, sectionmaxsize);

  QDTPDEBUG << "sectionValue for" << sn.name() << "with text" << m_text << "and (at" << offset
            << ") st:" << sectionTextRef;
  ...
  switch (sn.type) {
    ...
    case TimeZoneSection:
      result =
        findTimeZone(sectionTextRef, defaultValue, absoluteMax(sectionIndex), absoluteMin(sectionIndex), sn.count);
      break;
    case MonthSection:
    case DayOfWeekSectionShort:
    case DayOfWeekSectionLong:
    ...
    case SecondSection:
    case MSecSection: {
      const auto checkSeparator =
        [&result, field = QStringView{m_text}.sliced(offset), negativeYearOffset, sectionIndex, this]() {
      ...
      if (sectionTextRef.startsWith(u'-') || sectionTextRef.startsWith(u'+')) {
      ...
      if (digitsStr.isEmpty()) {
        ...
        if (lastVal == -1) {
          if (!checkSeparator()) {
            QDTPDEBUG << "invalid because" << sectionTextRef << "can't become a uint" << lastVal;
          }
        }
        else {
          if (negate)
          ...
          if (negate ? lastVal > absMax : lastVal < absMin) {
            if (unfilled) {
              ...
              QDTPDEBUG << "invalid because" << lastVal << "is greater than absoluteMax" << absMax;
            }
            else {
              QDTPDEBUG << "invalid because" << lastVal << "is less than absoluteMin" << absMin;
            }
          }
    ...
    default:
      qWarning("QDateTimeParser::parseSection Internal error (%ls %d)", qUtf16Printable(sn.name()), sectionIndex);
      return result;
  }
...
static QDate actualDate(QDateTimeParser::Sections known,
  ...
  int last =
    known & QDateTimeParser::MonthSection
      ? (known & QDateTimeParser::YearSection ? calendar.daysInMonth(month, year) : calendar.daysInMonth(month))
      : 0;
  // We can only fix DOW if we know year as well as month (hence last):
  const bool fixDayOfWeek =
    last && known & QDateTimeParser::YearSection && known & QDateTimeParser::DayOfWeekSectionMask;
  ...
  if (!actual.isValid() // We can't do better than we have, in this case
      || (known & QDateTimeParser::DaySection && known & QDateTimeParser::MonthSection &&
...
static QTime actualTime(QDateTimeParser::Sections known,
  ...
  if (ampm == -1 || (known & QDateTimeParser::AmPmSection) == 0) {
    ...
    if ((known & QDateTimeParser::Hour24Section) == 0 && known & QDateTimeParser::Hour12Section) {
      hour = hour12 + ampm * 12;
    }
  }
...
/*!
  \internal
*/
QDateTimeParser::StateNode QDateTimeParser::scanString(const QDateTime& defaultValue, bool fixup) const {
  ...
  for (int index = 0; index < sectionNodesCount; ++index) {
    ...
    if (step == -1) {
      QDTPDEBUG << "invalid because" << QStringView{m_text}.sliced(pos) << "does not start with" << separator << index
                << pos << currentSectionIndex;
      return StateNode();
    }
    pos += step;
    ...
    const QDateTime usedDateTime = [&] {
      const QDate date = actualDate(isSet, calendar, year, year2digits, month, day, dayofweek);
    ...
    QDTPDEBUG << "sectionValue" << sn.name() << m_text << "pos" << pos << "used" << sect.used << stateName(sect.state);
    ...
    switch (sn.type) {
      case TimeZoneSection:
        ...
        if (sect.used > 0) {
          ...
          const QStringView offsetStr = zoneName.startsWith("UTC"_L1) ? zoneName.sliced(3) : zoneName;
      ...
      case DaySection:
        current = &day;
        sect.value = qMax<int>(1, sect.value);
        break;
      case AmPmSection:
        current = &ampm;
        break;
      default:
        qWarning("QDateTimeParser::parse Internal error (%ls)", qUtf16Printable(sn.name()));
        return StateNode();
    }
    Q_ASSERT(current);
    ...
    if (sect.used > 0)
      pos += sect.used;
    QDTPDEBUG << index << sn.name() << "is set to" << pos << "state is" << stateName(state);
  ...
  if (step == -1 /* || step + pos < m_text.size()*/) {
    QDTPDEBUG << "invalid because" << QStringView{m_text}.sliced(pos) << "does not match" << separators.last() << pos;
    return StateNode();
  }
  ...
  if (parserType != QMetaType::QTime) {
    ...
    if ((!date.isValid() || dayofweek != calendar.dayOfWeek(date)) && state == Acceptable &&
      ...
      if (currentSectionIndex == -1 || fieldType & DayOfWeekSectionMask ||
        ...
        day = weekDayWithinMonth(calendar, year, month, day, dayofweek);
        QDTPDEBUG << year << month << day << dayofweek << calendar.dayOfWeek(QDate(year, month, day, calendar));
      }
    }
        ...
        for (int i = 0; i < sectionNodesCount; ++i) {
          ...
          if (sn.type & DaySection) {
            ...
            const QLocale::FormatType dayFormat =
              (sn.type == DayOfWeekSectionShort ? QLocale::ShortFormat : QLocale::LongFormat);
  ...
  if (!(isSet & HourSectionMask) && !when.isValid()) {
    switch (parserType) {
      case QMetaType::QDateTime: {
        ...
        if (replace.date() == date && (!(isSet & MinuteSection) || tick.minute() == minute) &&
            (!(isSet & SecondSection) || tick.second() == second) && (!(isSet & MSecSection) || tick.msec() == msec)) {
    ...
    if (context != FromString && scan.value < minimum) {
      ...
      if (scan.value >= minimum)
        qWarning("QDateTimeParser::parse Internal error 3 (%ls %ls)",
                 qUtf16Printable(scan.value.toString()),
                 qUtf16Printable(minimum.toString()));
      ...
      for (int i = 0; i < sectionNodesCount && !done; ++i) {
        ...
        if ((t.size() < sectionMaxSize(i) && ((fieldInfo(i) & (FixedWidth | Numeric)) != Numeric)) ||
            t.contains(space)) {
          switch (sn.type) {
            case AmPmSection:
              switch (findAmPm(t, i)) {
            ...
            default: {
              ...
              if (toMin > maxChange) {
                QDTPDEBUG << "invalid because toMin > maxChange" << toMin << maxChange << t << scan.value << minimum;
                scan.state = Invalid;
                done = true;
                break;
              }
              ...
              if (min == -1) {
                qWarning("QDateTimeParser::parse Internal error 4 (%ls)", qUtf16Printable(sn.name()));
                scan.state = Invalid;
                done = true;
                break;
              }
              ...
              if (!potentialValue(t.simplified(), min, max, i, scan.value, pos)) {
                QDTPDEBUG << "invalid because potentialValue(" << t.simplified() << min << max << sn.name()
...
QDateTimeParser::ParsedSection QDateTimeParser::findUtcOffset(QStringView str, int mode) const {
  ...
  const State status = (hours > 14 || minutes >= 60)  ? Invalid
                       : (hours == 14 && minutes > 0) ? Intermediate
                                                      : Acceptable;
  ...
  const int usedSymbols = (startsWithUtc ? 3 : 0) + 1 + hoursLength + (hasColon ? 1 : 0) + minutesStr.size();
 ...
 */
QDateTimeParser::ParsedSection QDateTimeParser::findTimeZoneName(QStringView str, const QDateTime& when) const {
  ...
  int index = std::distance(str.cbegin(), std::find_if(str.cbegin(), str.cend(), invalidZoneNameCharacter));
...
/*!
  ...
  * 4: only a zone name
*/
QDateTimeParser::ParsedSection QDateTimeParser::findTimeZone(QStringView str,
    ...
    if (ch != space) {
      for (int j = 0; j < 2; ++j) {
        if (!broken[j]) {
          ...
          QDTPDEBUG << "looking for" << ch << "in" << ampm[j] << "and got" << index;
          if (index == -1) {
            if (ch.category() == QChar::Letter_Uppercase) {
              index = ampm[j].indexOf(ch.toLower());
              QDTPDEBUG << "trying with" << ch.toLower() << "in" << ampm[j] << "and got" << index;
            }
            else if (ch.category() == QChar::Letter_Lowercase) {
              index = ampm[j].indexOf(ch.toUpper());
              QDTPDEBUG << "trying with" << ch.toUpper() << "in" << ampm[j] << "and got" << index;
            }
            if (index == -1) {
              broken[j] = true;
...
int QDateTimeParser::SectionNode::maxChange() const {
  switch (type) {
    ...
    case YearSection2Digits:
      return 100 * 365;
    default:
      qWarning("QDateTimeParser::maxChange() Internal error (%ls)", qUtf16Printable(name()));
  }
...
QDateTimeParser::FieldInfo QDateTimeParser::fieldInfo(int index) const {
  ...
  switch (sn.type) {
    ...
    case AmPmSection:
      ...
      if (getAmPmText(AmText, Case(sn.count)).size() == getAmPmText(PmText, Case(sn.count)).size()) {
    ...
    case TimeZoneSection:
      break;
    default:
      qWarning("QDateTimeParser::fieldInfo Internal error 2 (%d %ls %d)", index, qUtf16Printable(sn.name()), sn.count);
      break;
  }
  return ret;
}
...
QString QDateTimeParser::SectionNode::format() const {
  ...
  switch (type) {
    case AmPmSection:
      return count == 1 ? "ap"_L1 : count == 2 ? "AP"_L1 : "Ap"_L1;
    case MSecSection:
      fillChar = u'z';
      break;
    case SecondSection:
    ...
    case YearSection:
      ...
      break;
    default:
      qWarning("QDateTimeParser::sectionFormat Internal error (%ls)", qUtf16Printable(name(type)));
      return QString();
  }
  if (fillChar.isNull()) {
...
bool QDateTimeParser::potentialValue(QStringView str,
                                     int min,
                                     int max,
                                     int index,
...
QString QDateTimeParser::SectionNode::name(QDateTimeParser::Section s) {
  switch (s) {
    ...
    case QDateTimeParser::TimeZoneSection:
    ...
    default:
      return "Unknown section "_L1 + QString::number(int(s));
  }
}
