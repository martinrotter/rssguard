  ...
  if (!m_root.isNull()) {
    ...
    if (!same_message) {
      ...
      if (custom_previewer != nullptr) {
        ...
        ensureDefaultBrowserVisible();
        m_msgBrowser->loadMessage({message}, root, feed);
      }
    }
  }
}
...
void MessagePreviewer::markMessageAsReadUnread(RootItem::ReadStatus read) {
  if (!m_root.isNull()) {
    ...
    qApp->database()->worker()->write([&](const QSqlDatabase& db) {
      DatabaseQueries::markMessagesReadUnread(db, {QString::number(m_message.m_id)}, read);
    });
...
void MessagePreviewer::switchMessageImportance(bool checked) {
  if (!m_root.isNull()) {
    ...
    qApp->database()->worker()->write([&](const QSqlDatabase& db) {
      DatabaseQueries::switchMessagesImportance(db, {QString::number(m_message.m_id)});
    });
