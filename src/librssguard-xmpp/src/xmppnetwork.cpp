...
XmppNetwork::XmppNetwork(QObject* parent)
  : QObject(parent), m_root(nullptr), m_service(XmppServiceRoot::Service::FreshRss), m_username(QString()),
    m_password(QString()), m_baseUrl(QString()), m_batchSize(XMPP_DEFAULT_BATCH_SIZE),
...
QNetworkReply::NetworkError XmppNetwork::editLabels(const QString& state,
                                                       bool assign,
  ...
  working_subset.reserve(std::min(XMPP_API_EDIT_TAG_BATCH, int(trimmed_ids.size())));

  // Now, we perform messages update in batches (max X messages per batch).
  while (!trimmed_ids.isEmpty()) {
    // We take X IDs.
    for (int i = 0; i < XMPP_API_EDIT_TAG_BATCH && !trimmed_ids.isEmpty(); i++) {
    ...
    if (m_service == XmppServiceRoot::Service::Reedah || m_service == XmppServiceRoot::Service::Miniflux) {
    ...
    if (result_edit.m_networkError != QNetworkReply::NetworkError::NoError) {
      qCriticalNN << LOGSEC_XMPP << "Result of edit-tag operation is"
...
QVariantHash XmppNetwork::userInfo(const QNetworkProxy& proxy) {
...
void XmppNetwork::prepareFeedFetching(XmppServiceRoot* root,
  ...
  m_performGlobalFetching = perc_of_fetching > XMPP_GLOBAL_UPDATE_THRES;

  qDebugNN << LOGSEC_XMPP << "Percentage of feeds for fetching:" << QUOTE_W_SPACE_DOT(perc_of_fetching * 100.0);

  auto remote_starred_ids_list = itemIds(QSL(XMPP_API_FULL_STATE_IMPORTANT), false, proxy, -1, m_newerThanFilter);
  ...
  if (m_performGlobalFetching) {
    qWarningNN << LOGSEC_XMPP << "Performing global contents fetching.";

    QStringList remote_all_ids_list =
      m_downloadOnlyUnreadMessages
        ? QStringList()
        : itemIds(QSL(XMPP_API_FULL_STATE_READING_LIST), false, proxy, -1, m_newerThanFilter);
    QStringList remote_unread_ids_list =
      itemIds(QSL(XMPP_API_FULL_STATE_READING_LIST), true, proxy, -1, m_newerThanFilter);
    ...
    qWarningNN << LOGSEC_XMPP << "Performing feed-based contents fetching.";
  }

  QList<QString> to_download_list(to_download.values());

  if (!to_download_list.isEmpty()) {
    if (m_service == XmppServiceRoot::Service::Reedah) {
...
QList<Message> XmppNetwork::getMessagesIntelligently(ServiceRoot* root,
...
QNetworkReply::NetworkError XmppNetwork::markMessagesRead(RootItem::ReadStatus status,
                                                             const QStringList& msg_custom_ids,
                                                             const QNetworkProxy& proxy) {
  return editLabels(QSL(XMPP_API_FULL_STATE_READ), status == RootItem::ReadStatus::Read, msg_custom_ids, proxy);
}

QNetworkReply::NetworkError XmppNetwork::markMessagesStarred(RootItem::Importance importance,
                                                                const QStringList& msg_custom_ids,
                                                                const QNetworkProxy& proxy) {
  return editLabels(QSL(XMPP_API_FULL_STATE_IMPORTANT),
                    importance == RootItem::Importance::Important,
                    msg_custom_ids,
                    proxy);
}

void XmppNetwork::subscriptionEdit(const QString& op,
                                      const QString& stream_id,
  ...
  if (op == QSL(XMPP_API_EDIT_SUBSCRIPTION_ADD)) {
  ...
  if (op == QSL(XMPP_API_EDIT_SUBSCRIPTION_MODIFY)) {
    full_url += QSL("&t=%1").arg(new_title);
  ...
  if (result.m_networkError != QNetworkReply::NetworkError::NoError) {
    qCriticalNN << LOGSEC_XMPP << "Cannot edit subscription:" << QUOTE_W_SPACE_DOT(result.m_networkError);
    throw NetworkException(result.m_networkError, output);
  }
}

void XmppNetwork::subscriptionImport(const QByteArray& opml_data, const QNetworkProxy& proxy) {
  if (!ensureLogin(proxy)) {
    throw ApplicationException(tr("login failed"));
  }
  ...
  if (result.m_networkError != QNetworkReply::NetworkError::NoError) {
    qCriticalNN << LOGSEC_XMPP << "Cannot get OPML data, network error:" << QUOTE_W_SPACE_DOT(result.m_networkError);
    throw NetworkException(result.m_networkError, output);
  }
}

QByteArray XmppNetwork::subscriptionExport(const QNetworkProxy& proxy) {
  if (!ensureLogin(proxy)) {
    throw ApplicationException(tr("login failed"));
  }
...
QStringList XmppNetwork::itemIds(const QString& stream_id,
                                    bool unread_only,
                                    const QNetworkProxy& proxy,
                                    int max_count,
                                    QDate newer_than) {
  ...
  do {
    QString full_url =
      generateFullUrl(Operations::ItemIds)
        .arg(m_service == XmppServiceRoot::Service::TheOldReader ? stream_id : QUrl::toPercentEncoding(stream_id),
             QString::number(max_count <= 0 ? XMPP_API_ITEM_IDS_MAX : max_count));
    auto timeout = qApp->settings()->value(GROUP(Feeds), SETTING(Feeds::UpdateTimeout)).toInt();

    if (unread_only) {
      full_url += QSL("&xt=%1").arg(QSL(XMPP_API_FULL_STATE_READ));
    }
    ...
    if (result_stream.m_networkError != QNetworkReply::NetworkError::NoError) {
      qCriticalNN << LOGSEC_XMPP << "Cannot download item IDs for " << QUOTE_NO_SPACE(stream_id)
...
QList<Message> XmppNetwork::itemContents(ServiceRoot* root,
  ...
  while (!my_stream_ids.isEmpty()) {
    int batch =
      (m_service == XmppServiceRoot::Service::TheOldReader || m_service == XmppServiceRoot::Service::FreshRss)
        ? TOR_ITEM_CONTENTS_BATCH
        : (m_service == XmppServiceRoot::Service::Inoreader ? INO_ITEM_CONTENTS_BATCH
                                                               : XMPP_API_ITEM_CONTENTS_BATCH);
    ...
    do {
      ...
      auto inp = qlinq::from(batch_ids)
                   .select([this](const QString& id) {
                     return QSL("i=%1").arg(m_service == XmppServiceRoot::Service::TheOldReader
                                              ? id
      ...
      if (result_stream.m_networkError != QNetworkReply::NetworkError::NoError) {
        qCriticalNN << LOGSEC_XMPP << "Cannot download messages for " << batch_ids
                    << ", network error:" << QUOTE_W_SPACE_DOT(result_stream.m_networkError);
        throw NetworkException(result_stream.m_networkError, output_stream);
      }
      else {
...
QList<Message> XmppNetwork::streamContents(ServiceRoot* root, const QString& stream_id, const QNetworkProxy& proxy) {
  QString continuation;
  ...
  do {
    QString full_url = generateFullUrl(Operations::StreamContents)
                         .arg((m_service == XmppServiceRoot::Service::TheOldReader ||
                               m_service == XmppServiceRoot::Service::FreshRss)
                                ? stream_id
    ...
    if (result_stream.m_networkError != QNetworkReply::NetworkError::NoError) {
      qCriticalNN << LOGSEC_XMPP << "Cannot download messages for " << QUOTE_NO_SPACE(stream_id)
                  << ", network error:" << QUOTE_W_SPACE_DOT(result_stream.m_networkError);
      throw NetworkException(result_stream.m_networkError, output_stream);
    }
    else {
...
RootItem* XmppNetwork::categoriesFeedsLabelsTree(bool obtain_icons, const QNetworkProxy& proxy) {
  QString full_url = generateFullUrl(Operations::TagList);
  auto timeout = qApp->settings()->value(GROUP(Feeds), SETTING(Feeds::UpdateTimeout)).toInt();

  if (!ensureLogin(proxy)) {
    qCriticalNN << LOGSEC_XMPP << "Cannot get feed tree, not logged-in.";
    throw ApplicationException(tr("login failed"));
  }
...
RootItem* XmppNetwork::decodeTagsSubscriptions(const QString& categories,
                                                  const QString& feeds,
                                                  bool obtain_icons,
                                                  const QNetworkProxy& proxy) {
  ...
  if (m_service == XmppServiceRoot::Service::Bazqux || m_service == XmppServiceRoot::Service::Reedah ||
      m_service == XmppServiceRoot::Service::Inoreader) {
  ...
  for (const QJsonValue& obj : std::as_const(json)) {
    ...
    if ((label[QSL("type")].toString() == QSL("folder") ||
         (m_service == XmppServiceRoot::Service::TheOldReader && label_id.contains(QSL("/label/"))))) {
    ...
    else if ((m_service == XmppServiceRoot::Service::Bazqux || m_service == XmppServiceRoot::Service::Reedah ||
              m_service == XmppServiceRoot::Service::Inoreader) &&
  ...
  for (const QJsonValue& obj : std::as_const(json)) {
    ...
    if (obtain_icons) {
      ...
      if (!icon_url.isEmpty()) {
        ...
        else if (service() == XmppServiceRoot::Service::FreshRss) {
...
QNetworkReply::NetworkError XmppNetwork::clientLogin(const QNetworkProxy& proxy) {
  ...
  qDebugNN << LOGSEC_XMPP << "Arguments for login:" << QUOTE_W_SPACE_DOT(args);
  qDebugNN << LOGSEC_XMPP << "Full loging URL:" << QUOTE_W_SPACE_DOT(full_url);
  ...
  qDebugNN << LOGSEC_XMPP << "Login network result:" << QUOTE_W_SPACE_DOT(network_result.m_httpCode);
  qDebugNN << LOGSEC_XMPP << "Login response data:" << QUOTE_W_SPACE_DOT(output);
...
XmppServiceRoot::Service XmppNetwork::service() const {
  return m_service;
}

void XmppNetwork::setService(XmppServiceRoot::Service service) {
  m_service = service;
}
...
QString XmppNetwork::baseUrl() const {
  return m_baseUrl;
}

void XmppNetwork::setBaseUrl(const QString& base_url) {
  m_baseUrl = base_url;
}

QPair<QByteArray, QByteArray> XmppNetwork::authHeader() const {
  if (m_service == XmppServiceRoot::Service::Inoreader) {
...
QString XmppNetwork::tokenParameter() const {
  return QSL("T=%1").arg(m_authToken);
}

bool XmppNetwork::ensureLogin(const QNetworkProxy& proxy, QNetworkReply::NetworkError* output) {
  if (m_service == XmppServiceRoot::Service::Inoreader) {
  ...
  if (m_authSid.isEmpty() && m_authAuth.isEmpty()) {
    ...
    if (login != QNetworkReply::NetworkError::NoError) {
      ...
      qDebugNN << LOGSEC_XMPP << "Login successful.";
    }
  }
...
QString XmppNetwork::convertLongStreamIdToShortStreamId(const QString& stream_id) const {
  return QString::number(QString(stream_id)
                           .replace(QSL("tag:google.com,2005:reader/item/"), QString())
                           .toULongLong(nullptr, 16));
}

QString XmppNetwork::convertShortStreamIdToLongStreamId(const QString& stream_id) const {
  if (stream_id.startsWith(QSL("tag:google.com,2005:reader/item/"))) {
    return stream_id;
  }

  if (m_service == XmppServiceRoot::Service::TheOldReader) {
...
QStringList XmppNetwork::decodeItemIds(const QString& stream_json_data, QString& continuation) {
  QStringList ids;
...
QList<Message> XmppNetwork::decodeStreamContents(ServiceRoot* root,
  ...
  for (const QJsonValue& obj : std::as_const(json)) {
    ...
    for (const QJsonValue& cat : std::as_const(categories)) {
      ...
      if (category.endsWith(QSL(XMPP_API_STATE_READ))) {
        message.m_isRead = true;
      }
      else if (category.endsWith(QSL(XMPP_API_STATE_IMPORTANT))) {
...
int XmppNetwork::batchSize() const {
  return m_batchSize;
}

void XmppNetwork::setBatchSize(int batch_size) {
  m_batchSize = batch_size;
}

void XmppNetwork::clearCredentials() {
  m_authAuth = m_authSid = m_authToken = QString();
}

void XmppNetwork::clearPrefetchedMessages() {
  m_prefetchedMessages.clear();
}

QString XmppNetwork::sanitizedBaseUrl() const {
  QString base_url = m_service == XmppServiceRoot::Service::Inoreader ? QSL(XMPP_URL_INOREADER) : m_baseUrl;
  ...
  switch (m_service) {
    case XmppServiceRoot::Service::FreshRss:
...
QString XmppNetwork::generateFullUrl(XmppNetwork::Operations operation) const {
  switch (operation) {
    case Operations::ClientLogin:
      return sanitizedBaseUrl() + QSL(XMPP_API_CLIENT_LOGIN);

    case Operations::SubscriptionExport:
      return sanitizedBaseUrl() + QSL(XMPP_API_SUBSCRIPTION_EXPORT);

    case Operations::SubscriptionImport:
      return sanitizedBaseUrl() + QSL(XMPP_API_SUBSCRIPTION_IMPORT);

    case Operations::SubscriptionEdit:
      return sanitizedBaseUrl() + QSL(XMPP_API_SUBSCRIPTION_EDIT);

    case Operations::Token:
      return sanitizedBaseUrl() + QSL(XMPP_API_TOKEN);

    case Operations::TagList:
      return sanitizedBaseUrl() + QSL(XMPP_API_TAG_LIST);

    case Operations::SubscriptionList:
      return sanitizedBaseUrl() + QSL(XMPP_API_SUBSCRIPTION_LIST);

    case Operations::StreamContents:
      return sanitizedBaseUrl() + QSL(XMPP_API_STREAM_CONTENTS);

    case Operations::UserInfo:
      return sanitizedBaseUrl() + QSL(XMPP_API_USER_INFO);

    case Operations::EditTag:
      return sanitizedBaseUrl() + QSL(XMPP_API_EDIT_TAG);

    case Operations::ItemIds:
      return sanitizedBaseUrl() + QSL(XMPP_API_ITEM_IDS);

    case Operations::ItemContents:
      return sanitizedBaseUrl() + QSL(XMPP_API_ITEM_CONTENTS);
...
void XmppNetwork::onTokensError(const QString& error, const QString& error_description) {
  Q_UNUSED(error)
...
void XmppNetwork::initializeOauth() {
  ...
  connect(m_oauth, &OAuth2Service::tokensRetrieveError, this, &XmppNetwork::onTokensError);
  connect(m_oauth, &OAuth2Service::authFailed, this, &XmppNetwork::onAuthFailed);
  connect(m_oauth,
...
QDate XmppNetwork::newerThanFilter() const {
  return m_newerThanFilter;
}

void XmppNetwork::setNewerThanFilter(const QDate& newer_than) {
  m_newerThanFilter = newer_than;
}

OAuth2Service* XmppNetwork::oauth() const {
  return m_oauth;
}

void XmppNetwork::setOauth(OAuth2Service* oauth) {
  m_oauth = oauth;
}

void XmppNetwork::setRoot(XmppServiceRoot* root) {
  m_root = root;
}

bool XmppNetwork::intelligentSynchronization() const {
  return m_intelligentSynchronization;
}

void XmppNetwork::setIntelligentSynchronization(bool intelligent_synchronization) {
  m_intelligentSynchronization = intelligent_synchronization;
}

bool XmppNetwork::downloadOnlyUnreadMessages() const {
  return m_downloadOnlyUnreadMessages;
}

void XmppNetwork::setDownloadOnlyUnreadMessages(bool download_only_unread) {
